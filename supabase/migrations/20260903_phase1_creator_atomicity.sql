...
begin
  ...
  if to_regclass('public.creator_earnings') is not null then
    if p_post_id is not null then
      execute 'insert into public.creator_earnings(user_id,source,amount,post_id,status) values ($1,$2,$3,$4,$5) returning id' into v_creator_earning using p_user_id,p_source,p_amount,p_post_id,'completed';
    else
      execute 'insert into public.creator_earnings(user_id,source,amount,status) values ($1,$2,$3,$4) returning id' into v_creator_earning using p_user_id,p_source,p_amount,'completed';
    end if;
  end if;
